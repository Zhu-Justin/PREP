...
rows = strfind(patient_labels.ID,'kav');
rows = find(~cellfun('isempty',rows));
vars = {'ID','Group_A_Study_B_control_','WalkingTests'};

patient_table = patient_labels(rows,vars);
%patient_array = table2array(patient_table);
[patient_number,~] = size(rows);
table2array(patient_labels(:,1));

all_tests = sum(patient_labels.WalkingTests(rows));
all_subjects = strings(all_tests,1);
subject_counter = 1
for i = 1:patient_number
    subject = char(strcat(patient_table.ID(i),patient_table.Group_A_Study_B_control_(i)));
    if patient_table.WalkingTests(i) > 1
        total_walks = patient_table.WalkingTests(i);
        for walk = 1:total_walks
            strwalk = int2str(walk);
            subject(4) = strwalk;
            all_subjects(subject_counter) = subject;
...
patient_tests = size(all_subjects);
